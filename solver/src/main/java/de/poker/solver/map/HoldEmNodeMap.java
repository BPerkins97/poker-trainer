...
public class HoldEmNodeMap {
    ...
    public ActionMap getNodeForCurrentPlayer(HoldEmGameTree gameTree) {
        ...
        if (Objects.isNull(tempMap)) {
            tempMap = getOrPut(gameTree);
        }

        ActionMap node = tempMap.get(gameTree.history());
        if  (Objects.isNull(node)) {
            node = getOrPut(tempMap, gameTree);

        }
        return node;
    }

    private ActionMap getOrPut(Map<String, ActionMap> tempMap, HoldEmGameTree gameTree) {
        synchronized (tempMap) {
            ActionMap actionMap = tempMap.get(gameTree.history());
            if (Objects.isNull(actionMap)) {
                actionMap = new ActionMap();
                tempMap.put(gameTree.history(), actionMap);
            }
            return actionMap;
        }
    }

    private Map<String, ActionMap> getOrPut(HoldEmGameTree gameTree) {
        synchronized (map[gameTree.bettingRound][gameTree.currentPlayer]) {
            Map<String, ActionMap> tempMap = map[gameTree.bettingRound][gameTree.currentPlayer].get(gameTree.cardInfoSets[gameTree.bettingRound][gameTree.currentPlayer]);
            if (Objects.isNull(tempMap)) {
                tempMap = new HashMap<>();
            ...
            for (int j=0;j<Constants.NUM_PLAYERS;j++) {
                for (Map.Entry<String, Map<String, ActionMap>> entry1 : map[i][j].entrySet()) {
                    for (Map.Entry<String, ActionMap> entry2 : entry1.getValue().entrySet()) {
                        for (Map.Entry<Action, Node> entry3 : entry2.getValue().getMap().entrySet()) {
                            String output = i + ";" + j + ";" + entry1.getKey() + ";" + entry2.getKey() + ";" + entry3.getKey().presentation() + ";" + entry3.getValue().getRegret() + ";" + entry3.getValue().getAverageAction() + "\r\n";
    ...
    public synchronized void loadFromFile(File file) throws IOException {
        ...
        while (!Objects.isNull(line = reader.readLine())) {
            ...
            Node value = new Node(regret, averageAction);
            actionMap.getMap().put(Action.of(myAction), value);
        }
        reader.close();
    }
}
